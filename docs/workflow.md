...
`/agent-spec`. It reads the pipeline state, picks one skill, says why in a line, and hands
off. Everything below is what it picks between.

## First session

Type `/agent-spec-onboard`. It fills `PROJECT-INDEX.md` and `CONSTITUTION.md` from the repo and then
...
Then `/agent-spec-sdlc`, once per gate. It reads the state, runs the gate that is due and stops —
`/agent-spec-requirements` first, and each gate names its successor rather than running it. Give
...
loud, run `/agent-spec-implement` directly, and record no gate you did not run.

You can still drive a single skill by hand, with a persona:

> *"Activate: @ARCHITECT. Run `/agent-spec-prd` for a password reset feature."*
>
> *"Activate: @QA. Run `/agent-spec-review` on `AuthService.java`."*
...
`/agent-spec-remember` for a fact that will still be true in three months — a constraint
someone imposed, a decision and its reason, a trap that already cost time. It goes in front
of every later session automatically.

Not for anything the repository answers. The graph, `git log` and `CONVENTIONS.md` already
know the code; a copy of that in memory goes stale silently.

## Ending a session

1. `/agent-spec-index-project` — rebuild the dependency map if the architecture moved.
